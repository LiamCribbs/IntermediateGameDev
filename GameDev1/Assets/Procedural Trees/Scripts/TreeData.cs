...
public class TreeData
{
    ...
    public static TreeData GrowTreeOnNewThread(TreeSettings settings)
    {
        TreeData tree = new TreeData(settings);
        System.Threading.Thread t = new System.Threading.Thread(() => tree.Start());
        t.Start();
        return tree;
    }

    public static TreeData GrowTreeOnNewThread(TreeSettings settings, int seed)
    {
        TreeData tree = new TreeData(settings, seed);
        System.Threading.Thread t = new System.Threading.Thread(() => tree.Start());
        t.Start();
        return tree;
    }

    public TreeData(TreeSettings settings)
    {
    ...
    public TreeData(TreeSettings settings, int seed)
    {
        this.settings = settings;
        this.seed = seed;
        random = new Random(seed);
    ...
    public Sprite Draw()
    {
        ...
        Sprite sprite = Sprite.Create(texture, new Rect(0f, 0f, width, height), new Vector2(-colorMap.xMin / (float)width, -colorMap.yMin / (float)height), 100f, 1u, SpriteMeshType.FullRect);
